...
class SportEventControllerTest {
        ...
        class UpdateStatusTests {
                ...
                void shouldRejectActivationWithPastStartTime() throws Exception {
                        // Directly inject event with past startTime via service bypassing the
                        // controller
                        // We create with future time, then manually set past time via low-level
                        // approach
                        // Instead, we test via the service layer directly and validate the 422 via
                        // controller
                        ...
                        mockMvc.perform(patch("/api/events/{id}/status", created.getId())
                                        ...
                                        .andExpect(jsonPath("$.message")
                                                        .value(org.hamcrest.Matchers.containsString("past")));
                }
